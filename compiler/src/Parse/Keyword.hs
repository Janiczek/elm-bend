...
intrinsic :: (Row -> Col -> x) -> Parser x ()
intrinsic toError =
  -- [hex(ord(x)) for x in "__intrinsic"]
  P.Parser $ \(P.State src pos end indent row col) cok _ _ eerr ->
    let !pos11 = plusPtr pos 11 in
    if pos11 <= end
      && P.unsafeIndex (        pos   ) == 0x5F
      && P.unsafeIndex (plusPtr pos  1) == 0x5F
      && P.unsafeIndex (plusPtr pos  2) == 0x69
      && P.unsafeIndex (plusPtr pos  3) == 0x6E
      && P.unsafeIndex (plusPtr pos  4) == 0x74
      && P.unsafeIndex (plusPtr pos  5) == 0x72
      && P.unsafeIndex (plusPtr pos  6) == 0x69
      && P.unsafeIndex (plusPtr pos  7) == 0x6E
      && P.unsafeIndex (plusPtr pos  8) == 0x73
      && P.unsafeIndex (plusPtr pos  9) == 0x69
      && P.unsafeIndex (plusPtr pos 10) == 0x63
